...
export const AnalyticsService = {
  async getAnalytics(appIdFilter?: string) {
    const totalDevices = appIdFilter
      ? await sql`SELECT count(*)::int as count FROM devices WHERE app_id = ${appIdFilter}`
      : await sql`SELECT count(*)::int as count FROM devices`

    const versionDistribution = appIdFilter
      ? await sql`
          SELECT current_version as "version", count(*)::int as count
          FROM devices WHERE app_id = ${appIdFilter}
